...
parser.add_argument('--output_file',type=str,default='model.pth',help='path to model')
parser.add_argument('--data_dir',type=str,default='ner-gmb',help='path to data')
parser.add_argument('--glove_embeddings_file',type=str,default='glove/glove.6B.100d.txt',help='path to glove embeddings')
parser.add_argument('--vocabulary_output_file',type=str,default='words.vocab',help='path to vocab')
...
# wv = np.load('word_vocab.npy',allow_pickle='TRUE').item()
# cv = np.load('char_vocab.npy',allow_pickle='TRUE').item()
# lv = np.load('label_vocab.npy',allow_pickle='TRUE').item()
...
test_data = MyDataset(get_tensors(args.data_dir+'/test.txt',wv,cv,lv))
dataloaders = {}
...
a = args.output_file
def train_model(model,optimizer,num_epochs=args.epochs,patience=args.patience,use_crf=args.crf,name=a):
    clock=0
    since = time.time()
    ...
    for epoch in range(num_epochs):
        if(clock<patience):
          ...
          for phase in ['train', 'val','test']:
              ...
              for i,(s,c,t,sl,cl) in tqdm(enumerate(dataloaders[phase])):
                  max_word_len = sl.max()
                  max_char_len = cl.max()
                  s=s[:,:max_word_len]
                  ...
                  t=t[:,:max_word_len]
                  cl=cl[:,:max_word_len]
                  # print(c.shape,s.shape,t.shape,cl.shape)
                  # assert 1==2
                  s=s.to(device)
                  c=c.to(device)
                  t=t.to(device)
                  ...
                  with torch.set_grad_enabled(phase == 'train'):
                      if use_crf:
                        scores,predictions = model(s,c,t,sl)
                        loss = scores
                        # pred=[]
                        # true=[]
                        ...
                        # print(t*m-(1-m)*100)
                        # assert 1==2
                        masked_labels = t*m-(1-m)*100
              
                        loss = loss_fn(logits.reshape(bsz*seq_len,num_classes),masked_labels.reshape(bsz*seq_len))
                  ...
                  if phase == 'train' or not use_crf:
                    running_loss += loss.item() * sl.sum()
                  # running_macro_f1 += macro_f1* sl.sum()
                  # running_micro_f1 += micro_f1* sl.sum()
                  total_samples += sl.sum()
                  # total_it +=len(sl)
              f_pred,f_true = invert(pred,true)
                  
              epoch_macro_f1=f1_score(f_true,f_pred,average='macro')
              epoch_micro_f1=f1_score(f_true,f_pred,average='micro')
              epoch_acc = accuracy_score(f_true,f_pred)


              epoch_loss = running_loss / total_samples
              # epoch_macro_f1 = running_macro_f1 / total_samples
              # epoch_micro_f1 = running_micro_f1 / total_samples
# Loss: {:.4f} acc: {:.4f} mirco f1: {:.4f} macro f1: {:.4f}
              print('{} Loss: {:.4f} acc: {:.4f} micro f1: {:.4f} macro f1 {:.4f}'.format(
                  phase, epoch_loss,epoch_acc,epoch_micro_f1,epoch_macro_f1))

              if phase == 'val' and best_macro_f1<epoch_macro_f1:
    ...
    print('Best val f1: {:4f}'.format(best_macro_f1))


    model.load_state_dict(best_model_wts)
    return model

net = train_model(net,optimizer,use_crf=(args.crf==1),name=a)
